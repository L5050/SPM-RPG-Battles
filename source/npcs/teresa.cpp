...
namespace mod {
  ...
  EVT_BEGIN(teresa_onspawn)
    SET(LW(7), LW(1))
    ADD(LW(7), 20)
    USER_FUNC(evt_npc_set_position, LW(15), LW(0), LW(7), LW(2))
    USER_FUNC(spm::evt_npc::evt_npc_set_unitwork, LW(15), 1, LW(7))
    USER_FUNC(spm::evt_mobj::evt_mobj_set_position, LW(14), LW(0), LW(7), LW(2))
  RETURN()
  EVT_END()

  EVT_BEGIN(teresa_attack)
    SET(LW(0), 0)
    IF_EQUAL(LW(0), 0)
          ...
          USER_FUNC(spm::evt_snd::evt_snd_sfxon, PTR("SFX_E_TELESA_HIDE1"))
          SET(LW(8), 500)
          SET(LW(9), LW(15))
          RUN_EVT(make_npc_transparent)
          USER_FUNC(evt_npc_glide_to, LW(15), LW(0), LW(1), LW(2), 0, FLOAT(150.0), 0, 0, 0, 0)
          WAIT_FRM(1)
          USER_FUNC(evt_npc_set_anim, LW(15), 5, 1)
          USER_FUNC(evt_npc_wait_anim_end, LW(15), 1)
          RUN_EVT(make_npc_not_transparent)
          USER_FUNC(spm::evt_snd::evt_snd_sfxon, PTR("SFX_E_TELESA_HIDE1"))
          WAIT_MSEC(500)
